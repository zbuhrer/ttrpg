...
export default function StoryTimeline() {
  ...
  const createStoryBranchMutation = useMutation({
    mutationFn: async (data: any) => {
      const response = await apiRequest("POST", `/api/campaigns/${CAMPAIGN_ID}/story-branches`, data);
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/campaigns", CAMPAIGN_ID, "story-branches"] });
  ...
  const onSubmit = (data: any) => {
    createStoryBranchMutation.mutate(data);
  };

  const filteredBranches = storyBranches.filter((branch: any) =>
    branch.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
    branch.description?.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const activeBranches = filteredBranches.filter((b: any) => b.status === 'active');
  const pendingBranches = filteredBranches.filter((b: any) => b.status === 'pending');
  const dormantBranches = filteredBranches.filter((b: any) => b.status === 'dormant');

  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-fantasy font-bold text-fantasy-accent">Story & Timeline</h1>
          <p className="text-gray-400 mt-1">Manage narrative paths and story progression</p>
        </div>
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <div className="space-y-2">
                          <p className="text-sm text-gray-400">Select which characters are involved in this story branch</p>
                                ...
                                <input
                                  ...
                                  onChange={(e) => {
                                    const newValue = e.target.checked
                                      ...
                                      : field.value.filter((name: string) => name !== character.name);
                ...
                <div className="flex justify-end space-x-2">
                  ...
                  >
                    {createStoryBranchMutation.isPending ? "Creating..." : "Create Branch"}
                  </Button>
                </div>
              </form>
            </Form>
          ...
          <CardContent>
            {activeBranches.length > 0 ? (
              ...
              <div className="text-center py-8">
                ...
                <p className="text-sm text-gray-500 mt-1">Activate branches based on player decisions</p>
              </div>
            )}
          </CardContent>
        </Card>
          ...
          <CardContent>
            {pendingBranches.length > 0 ? (
              ...
              <div className="text-center py-8">
                ...
                <p className="text-sm text-gray-500 mt-1">Create branches for upcoming decisions</p>
              </div>
            )}
          </CardContent>
        </Card>
      ...
      {!isLoading && filteredBranches.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-sm text-gray-500 mt-1">
            {searchTerm ? "Try a different search term" : "Create your first story branch to track narrative paths"}
          </p>
        </div>
      )}
    </div>
