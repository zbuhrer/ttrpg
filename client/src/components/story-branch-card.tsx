...
export function StoryBranchCard({ storyBranch }: StoryBranchCardProps) {
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'active': return 'bg-fantasy-success/20 text-fantasy-success';
      case 'pending': return 'bg-fantasy-amber/20 text-fantasy-amber';
      case 'dormant': return 'bg-red-500/20 text-red-400';
  ...
  const getStatusIcon = (playerAlignment: string | null) => {
    switch (playerAlignment) {
      case 'all': return <Users className="text-fantasy-accent text-sm" />;
      case 'majority': return <Scale className="text-fantasy-accent text-sm" />;
      default: return <Clock className="text-gray-400 text-sm" />;
    }
  };
  ...
  return (
    <div className="p-4 bg-fantasy-dark/30 rounded-lg border border-fantasy-charcoal/50">
      ...
      {storyBranch.assignedCharacters && storyBranch.assignedCharacters.length > 0 && (
        <div className="mb-3">
          <p className="text-xs text-gray-500 mb-1">Assigned Characters:</p>
          <div className="flex flex-wrap gap-1">
            {storyBranch.assignedCharacters.map((characterName) => (
              <Badge key={characterName} variant="outline" className="text-xs text-fantasy-accent border-fantasy-accent/30">
                {characterName}
              </Badge>
            ))}
          </div>
