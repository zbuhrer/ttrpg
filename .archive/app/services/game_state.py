...
from .ai_service import AIResponse, CombatAction, combat_actions
...
class GameData(TypedDict, total=False):
    ...
    combat_state: Dict[str, Any]  # Add combat state
...
class GameStateManager:
    ...
    def generate_action_response(self,
        ...
        ai_response = self._safe_generate_response(
            ai_service,
            prompt=f"""
                ...
                *   Include a skill check if it is relevant to the action.
                ...
                """,
            fallback_message=AIResponse(content=f"You attempt to {action}, but the outcome is unclear...", success=False,
        ...
        for ca in combat_actions:
            if ca.name.lower() in action.lower():
                combat_action = ca
                break

        # Add combat action details to game_data if it's a combat action
        if combat_action:
            ai_response.metadata['combat_action'] = combat_action.name
            ai_response.xp += combat_action.xp_reward

        return ai_response  # Return the original AI response
    ...
    def start_combat(self, char_id: str) -> None:
        ...
        if game_state:
            game_data = game_state['game_data']  # Access game_data directly
