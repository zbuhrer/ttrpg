...
class CombatAction:
    ...
    name: str
    description: str
    target: str  # "enemy", "ally", "self", "location"
    type: str  # attack, defense, buff, debuff, item, etc.
    parameters: List[str]  # e.g., ["weapon", "spell"], ["potion", "scroll"]
...
combat_actions: List[CombatAction] = [
    CombatAction(
        name="Attack",
        description="Make an attack against a target.",
        target="enemy",
        type="attack",
        parameters=["weapon"],
        example_commands=[
            "I attack the goblin with my sword.",
            "Strike the orc with firebolt."
        ],
        xp_reward=15
    ),
    CombatAction(
        name="Defend",
        description="Defend yourself against incoming attacks.",
        target="self",
        type="defense",
        parameters=[],
        example_commands=[
            "I raise my shield to defend myself.",
            "I focus on dodging the attacks."
        ],
        xp_reward=5
    ),
    CombatAction(
        ...
        example_commands=[
            ...
            "I toss a smoke bomb at the enemy."
        ],
        xp_reward=10
    ),
...
class ResponseParser:
    ...
    def __init__(self):
        ...
        self.combat_actions = combat_actions  # Access the list of combat actions
    ...
    def parse_response(self, raw_response: str) -> AIResponse:
        ...
        try:
            ...
            # Determine combat action (if any)
            combat_action = self._determine_combat_action(content)
            xp = 0
            if combat_action:
                xp = combat_action.xp_reward
            ...
            return AIResponse(
                ...
                metadata={
                    ...
                    'combat_action': combat_action.name if combat_action else None
                },
                xp=xp
            )
    ...
    def _determine_combat_action(self, content: str) -> Optional[CombatAction]:
        """Determine if the response includes a combat action."""
        content_lower = content.lower()
        for action in self.combat_actions:
            for example in action.example_commands:
...
class AIService:
    ...
    def _construct_prompt(self,
        ...
        # Format combat actions
        formatted_actions = "\n".join([
            f"- {action.name}: {action.description} (Example: {
                action.example_commands[0]})"
            for action in self.combat_actions
        ])
